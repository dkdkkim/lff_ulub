...
def train(
    ...
    bias_criterion = GeneralizedCELoss(q=q)
    ...
    valid_attrwise_accs_list, test_attrwise_accs_list = [], []
    num_updated = 0
    ...
    for step in tqdm(range(main_num_steps)):
        ...
        if step % main_valid_freq == 0:
            valid_attrwise_accs_b, valid_acc_b = evaluate(model_b, valid_loader)
            # print(valid_attrwise_accs_b, valid_acc_b)
            test_attrwise_accs_b, test_acc_b = evaluate(model_b, test_loader)
            # print(test_attrwise_accs_b, test_acc_b)
            valid_attrwise_accs_d, valid_acc_d = evaluate(model_d, valid_loader)
            # print(valid_attrwise_accs_d, valid_acc_d)
            test_attrwise_accs_d, test_acc_d = evaluate(model_d, test_loader)
            # print(test_attrwise_accs_d, test_acc_d)
            ...
            writer.add_scalar(
                "acc/b_test_aligned",
                test_attrwise_accs_b[eye_tsr == 1].mean(),
                step,
            )
            writer.add_scalar(
                "acc/b_test_skewed",
                test_attrwise_accs_b[eye_tsr == 0].mean(),
                step,
            )
            writer.add_scalar(
                "acc/b_test",
            ...
            writer.add_scalar(
                "acc/d_test_aligned",
                test_attrwise_accs_d[eye_tsr == 1].mean(),
                step,
            )
            writer.add_scalar(
                "acc/d_test_skewed",
                test_attrwise_accs_d[eye_tsr == 0].mean(),
                step,
            )
            writer.add_scalar(
                "acc/d_test",
    ...
    test_attrwise_accs_list = torch.stack(test_attrwise_accs_list)
    with open(result_path, "wb") as f:
        torch.save({"valid/attrwise_accs": valid_attrwise_accs_list,
                    "test/attrwize_accs": test_attrwise_accs_list}, f)
    state_dict = {
        'steps': step, 
...
if __name__ == '__main__':
    ...
    for i in [2,3]:
        train(main_tag=f'utkface_ub1_ub2_{option.cls_type}_{option.exp_name}_0{i}',
                dataset_tag='utkface',
                model_tag='ResNet18',
                data_dir=option.data_path,
                log_dir=option.log_path,
                device=option.gpu,
                target_attr_idx=target_attr,
                bias_attr_idx=bias_attr,
                main_num_steps = 38 * option.epochs, # 88 * epoch
                main_valid_freq = 38,
                main_batch_size = 256,
        ...
        train(main_tag=f'utkface_ub2_ub1_{option.cls_type}_{option.exp_name}_0{i}',
                dataset_tag='utkface',
                model_tag='ResNet18',
                data_dir='/data/dk/ulub/dataset/utkface/utkcropped',
                log_dir=f"/home/dkkim/workspace/LfF_ulub/ulub/utkface/log",
                device=option.gpu,
                target_attr_idx=target_attr,
                bias_attr_idx=bias_attr,
                main_num_steps = 40 * option.epochs, # 88 * epoch
                main_valid_freq = 40,
                main_batch_size = 256,
