...
class ImageFolder(torch.utils.data.Dataset):
    def __init__(self, root, transform=None, target_transform=None, loader=pil_loader,
        ...
        self.clusters = []
        for i in range(3):
            self.clusters.append(torch.load('/data/dk/ulub/dataset/imagenet/clusters/cluster_label_{}.pth'.format(i+1)))
...
dataset = ImageFolder('/data/dk/ulub/dataset/imagenet/train', None)
print(len(dataset)) #54600


save_dir = 'dataset_ulub/imagenet'
...
for path, label in iter(dataset):
    imgs.append(path)
    lbls.append(label)
